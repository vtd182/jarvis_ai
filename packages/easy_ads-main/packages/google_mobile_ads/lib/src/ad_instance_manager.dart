...
class AdInstanceManager {
  ...
  Future<void> loadPreloadNativeAd(PreloadNativeAd ad) {
    if (adIdFor(ad) != null) {
      return Future<void>.value();
    }
  ...
  Future<RequestConfiguration> getRequestConfiguration() async {
    return (await instanceManager.channel
        .invokeMethod<RequestConfiguration>('MobileAds#getRequestConfiguration'))!;
  }
    ...
    switch (type) {
      ...
      case _valueAdRequest:
        return AdRequest(
          ...
          mediationExtras:
              readValueOfType(buffer.getUint8(), buffer)?.cast<List<MediationExtras>>(),
        );
      ...
      case _valueResponseInfo:
        return ResponseInfo(
          ...
          responseExtras:
              _deepCastStringKeyDynamicValueMap(readValueOfType(buffer.getUint8(), buffer)),
        );
      ...
      case _valueAdError:
        return AdError(readValueOfType(buffer.getUint8(), buffer),
            readValueOfType(buffer.getUint8(), buffer), readValueOfType(buffer.getUint8(), buffer));
